...
# torch.save(encoder1.state_dict(), data_dir + '/model.encoder.09apr04_en')
# torch.save(attn_decoder1.state_dict(), data_dir + '/model.decoder.09apr04_en')

encoder1 = EncoderRNN(input_lang.n_words, hidden_size).to(device)
encoder1.load_state_dict(torch.load(data_dir + '/model.encoder.09apr04_en', map_location=torch.device('cpu')))
#encoder1.eval()

attn_decoder1 = AttnDecoderRNN(hidden_size, output_lang.n_words, dropout_p=0.1).to(device)
attn_decoder1.load_state_dict(torch.load(data_dir + '/model.decoder.09apr04_en', map_location=torch.device('cpu')))
#attn_decoder1.eval()
